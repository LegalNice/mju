...
>
> 当前状态：**第一、二阶段已完成；第三阶段工作流已完成（前后端均已接入）；IA 重构（进入页 → Board → Dates → 任务子页）已完成；第四阶段交付物/DOCX 已完成；第五阶段通用化与开源准备已完成（5.1/5.3/5.4 完成，5.2 核心配置外置完成，Obsidian 路径映射延后）；第六阶段材料自动化 MVP 已完成；1.8.x 增强已完成——纸感视觉改版与圆珠笔蓝强调色（1.8.0）、案件驾驶舱（1.8.1）、确定性计算引擎 / 期限提议-确认闭环 / 看板待验收列（1.8.2）、看板拖拽与筛选、阶段补记与自定义阶段、Dates 月视图修复。**

## 目标

把当前以 pi-web 思维组织的 Mju Agents，重构为：

> **一个以法律工作流为中心、可开源的 Agent Teams 工作台。**
