...
import { colors, radius, microLabel } from "@/lib/design-system";
...
const UNREAD_SESSIONS_STORAGE_KEY = "mju:unread-session-ids";
...
function useScramble(target: string, running: boolean): string {
  ...
  useEffect(() => {
    ...
    iterRef.current = 0;
    const totalFrames = Math.max(target.length * 3, 12);
    const step = () => {
      iterRef.current += 1;
      const resolved = Math.floor((iterRef.current / totalFrames) * target.length);
...
function PiAgentTitle() {
  ...
  const target = showVersion
    ? `mju v${process.env.NEXT_PUBLIC_APP_VERSION ?? "0.0.0"} · pi ${process.env.NEXT_PUBLIC_PI_VERSION ?? "unknown"}`
    : "Mju Agents";
  const display = useScramble(target, scrambling);

  useEffect(() => {
    setScrambling(true);
    const timer = window.setTimeout(() => setScrambling(false), 520);
  ...
  return (
    <div
      ...
      title={showVersion ? "Mju 与内置 Pi 运行时版本" : "Mju Agents"}
      style={{
    ...
    >
      <span style={{ color: "var(--accent)", fontSize: 18, lineHeight: 1 }}>μ</span>
      {display}
    </div>
  );
}
...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onAtMention, onAtMentions }: Props) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Escape" && sessionSearchOpen) {
  ...
  const projectSessions = selectedProject
    ...
    : allSessions;
  // Keep history discoverable after starting a new session or closing the
  // current one. A workspace can legitimately have no saved sessions yet;
  // in that case fall back to the global history instead of showing an empty
  // sidebar and making older sessions appear lost.
  const sessionsForDisplay = projectSessions.length > 0 ? projectSessions : allSessions;
  const searchNeedle = sessionSearchQuery.trim().toLowerCase();
  const filteredSessions = searchNeedle
    ? sessionsForDisplay.filter((s) => [s.name, s.firstMessage, s.cwd].filter(Boolean).some((value) => value!.toLowerCase().includes(searchNeedle)))
    : sessionsForDisplay;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2, marginBottom: sessionSearchOpen ? 7 : 0 }}>
            <button
            ...
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 7, width: "100%", minWidth: 0, height: 31, padding: "0 9px", border: "1px solid var(--border)", borderRadius: radius.sm, background: sessionSearchOpen ? "var(--bg-hover)" : "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 12, textAlign: "left" }}
            >
              ...
              <span style={{ marginLeft: "auto", color: "var(--text-dim)", fontFamily: "var(--font-mono)", fontSize: 10 }}>⌘ K</span>
            </button>
        </div>
        {sessionSearchOpen && (
          <div style={{ position: "relative", marginBottom: 8 }}>
            <svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="var(--text-dim)" strokeWidth="1.5" strokeLinecap="round" style={{ position: "absolute", left: 10, top: 9, pointerEvents: "none" }} aria-hidden="true">
              <circle cx="7" cy="7" r="4.5" /><path d="m10.5 10.5 3 3" />
            </svg>
            <input
              ...
              style={{ width: "100%", height: 31, padding: "0 30px", border: "1px solid var(--accent)", borderRadius: radius.sm, background: "var(--bg)", color: "var(--text)", outline: "none", fontSize: 12 }}
            />
            {sessionSearchQuery && <button type="button" onClick={() => setSessionSearchQuery("")} aria-label="Clear search" style={{ position: "absolute", right: 7, top: 5, width: 21, height: 21, border: 0, borderRadius: radius.sm, background: "none", color: "var(--text-muted)", cursor: "pointer" }}>×</button>}
          </div>
        )}

        {/* CWD picker */}
        <div ref={dropdownRef} style={{ position: "relative", display: "none" }}>
          <button
            ...
            style={{
              ...
              alignItems: "center",
              padding: "6px 10px",
              background: selectedCwd ? "var(--bg-hover)" : colors.accentSofter,
              border: selectedCwd ? "1px solid var(--border)" : `1px solid ${colors.accent}`,
              borderRadius: radius.sm,
              cursor: "pointer",
              fontSize: 12,
              color: "var(--text)",
                  ...
                  <div style={{ maxHeight: "min(40vh, 300px)", overflowY: "auto" }}>
                    {worktreeState.worktrees.map((wt) => {
                      ...
                      if (wtConfirmRemove === wt.path) {
                        return (
                          <div key={wt.path} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 10px", borderBottom: "1px solid var(--border)", background: colors.errorSoft }}>
                            ...
                            <button
                              ...
                              disabled={wtBusy}
                              style={{ padding: "3px 9px", background: colors.error, border: "none", borderRadius: radius.sm, color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer", flexShrink: 0 }}
                            >
                              Force
                            </button>
                            <button
                              onClick={() => setWtConfirmRemove(null)}
                              style={{ padding: "3px 9px", background: "var(--bg-hover)", border: "1px solid var(--border)", borderRadius: radius.sm, color: "var(--text-muted)", fontSize: 11, cursor: "pointer", flexShrink: 0 }}
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        >
                          ...
                          {!wt.isMain && (
                            <button
                              ...
                              onMouseEnter={(e) => { e.currentTarget.style.color = colors.error; e.currentTarget.style.background = colors.errorSoft; }}
...
function RunningSessionIndicator() {
  return (
    ...
    >
      <span style={{ width: 6, height: 6, background: "var(--accent)", borderRadius: 0, display: "block" }} />
    </span>
  );
}
    ...
    <div
      ...
      style={{
        ...
        boxShadow: confirmDelete
          ? `inset 3px 0 0 ${colors.error}`
          : isSelected ? "inset 3px 0 0 var(--accent)" : "none",
        borderBottom: "1px solid var(--border)",
          ...
          <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: radius.sm, color: "var(--text-muted)",
