...
// Provider marks are deliberately kept in-app. This avoids a runtime icon
// dependency (and its peer-dependency installation warnings) while preserving
// a distinct, legible mark for the providers shown in the model picker.
const PROVIDER_BADGES: Record<string, { label: string; color: string; textColor?: string }> = {
  anthropic: { label: "AI", color: "#d97757" },
  openai: { label: "◎", color: "#101010" },
  "openai-codex": { label: "◎", color: "#101010" },
  google: { label: "G", color: "#4285f4" },
  "google-vertex": { label: "G", color: "#4285f4" },
  "ant-ling": { label: "ANT", color: "#1677ff" },
  deepseek: { label: "DS", color: "#4d6bfe" },
  groq: { label: "G", color: "#f55036" },
  mistral: { label: "M", color: "#ff7000", textColor: "#161616" },
  moonshotai: { label: "K", color: "#171717" },
  "moonshotai-cn": { label: "K", color: "#171717" },
  moonshot: { label: "K", color: "#171717" },
  minimax: { label: "MM", color: "#202020" },
  "minimax-cn": { label: "MM", color: "#202020" },
  fireworks: { label: "F", color: "#f05a28" },
  huggingface: { label: "🤗", color: "#ffca28", textColor: "#5d4400" },
  cerebras: { label: "C", color: "#ff5a1f" },
  openrouter: { label: "OR", color: "#665cff" },
  xai: { label: "𝕏", color: "#171717" },
  "cloudflare-ai-gateway": { label: "CF", color: "#f38020" },
  "cloudflare-workers-ai": { label: "CF", color: "#f38020" },
  "vercel-ai-gateway": { label: "▲", color: "#171717" },
  "github-copilot": { label: "GH", color: "#24292f" },
  "amazon-bedrock": { label: "aws", color: "#ff9900", textColor: "#1c252f" },
  "azure-openai-responses": { label: "AZ", color: "#0078d4" },
  "kimi-coding": { label: "K", color: "#171717" },
  nvidia: { label: "N", color: "#76b900", textColor: "#172800" },
  opencode: { label: "OC", color: "#6b52ff" },
  "opencode-go": { label: "OC", color: "#6b52ff" },
  qwen: { label: "Q", color: "#615ced" },
  xiaomi: { label: "mi", color: "#ff6900" },
  "xiaomi-token-plan-ams": { label: "mi", color: "#ff6900" },
  "xiaomi-token-plan-cn": { label: "mi", color: "#ff6900" },
  "xiaomi-token-plan-sgp": { label: "mi", color: "#ff6900" },
  zai: { label: "Z", color: "#2f73f8" },
  "zai-coding-cn": { label: "Z", color: "#2f73f8" },
  zhipu: { label: "Z", color: "#2f73f8" },
  cohere: { label: "C", color: "#39594d" },
  perplexity: { label: "P", color: "#20808d" },
  together: { label: "T", color: "#e6624f" },
  grok: { label: "𝕏", color: "#171717" },
};
...
function ProviderIcon({ id, size }: { id: string; size: number }) {
  const fallbackLabel = id
    ...
    .join("")
    .toUpperCase() || "?";
  const badge = PROVIDER_BADGES[id] ?? { label: fallbackLabel, color: "var(--bg-hover)" };
  return (
    <span
      aria-hidden="true"
      style={{
        width: size,
        height: size,
        border: badge.color === "var(--bg-hover)" ? "1px solid var(--border)" : "none",
        borderRadius: 4,
        ...
        justifyContent: "center",
        flexShrink: 0,
        fontSize: Math.max(8, Math.floor(size * (badge.label.length > 2 ? 0.31 : 0.43))),
        fontWeight: 700,
        lineHeight: 1,
      }}
    >
