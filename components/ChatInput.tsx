...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  const isNarrow = !isMobile && containerWidth !== null && containerWidth < 560;
  // 紧凑控制条：thinking / 工具预设 / Compact 收进弹层（移动端沿用同一机制）
  const compactControls = isMobile || isNarrow;
  ...
  // ResizeObserver 跟随分栏拖拽（320px ~ 60% 视口连续变化）
  useEffect(() => {
  ...
  const narrowModelName = currentName && currentName.length > 12
    ? `${currentName.slice(0, 12)}…`
    : currentName;

  // Stop / 声音在窄栏模式下保持图标常显：移动端留在弹层内（现状），非移动端移到弹层外
  const stopControl = (
    <button
      onClick={onAbort}
      title="Stop agent"
            ...
            {attachedImages.map((img, i) => (
              <div key={i} style={{ position: "relative", flexShrink: 0 }}>
                ...
                <img
                  src={img.previewUrl}
                  alt=""
                  style={{ width: 56, height: 56, objectFit: "cover", borderRadius: 2, border: "1px solid var(--border)", display: "block" }}
                />
                <button
        ...
        <div style={{ position: "relative" }}>
          ...
          {isStreaming ? (
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (value.trim() || attachedImages.length) e.currentTarget.style.background = "var(--accent-hover)";
              }}
              onMouseLeave={(e) => {
                ...
                <div ref={dropdownRef} style={{ position: "relative", flex: isMobile ? "1 1 auto" : undefined, minWidth: 0 }}>
                  <button
                    ...
                    style={{
                      ...
                      border: "none",
                      borderRadius: 2,
                      color: modelDropdownOpen ? "var(--accent)" : "var(--text-muted)",
                      cursor: isStreaming ? "not-allowed" : "pointer",
                      fontSize: 11,
                      fontWeight: 600,
                    ...
                    onMouseEnter={(e) => {
                      if (isStreaming) return;
                      e.currentTarget.style.background = "var(--bg-hover)";
                      e.currentTarget.style.color = modelDropdownOpen ? "var(--accent)" : "var(--text)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = modelDropdownOpen ? "var(--bg-hover)" : "none";
                      e.currentTarget.style.color = modelDropdownOpen ? "var(--accent)" : "var(--text-muted)";
                    }}
                  >
                    ...
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 }}>{isNarrow ? narrowModelName : currentName}</span>
                    {isNarrow && <span style={{ fontSize: 9, flexShrink: 0 }}>▾</span>}
                  </button>
                  {modelDropdownOpen && modelDropdownRect && (() => {
                    ...
                    return (
                      ...
                      borderRadius: 2, boxShadow: "0 8px 24px rgba(0,0,0,0.12)",
          ...
          <div ref={controlsMenuRef} style={{
            ...
            {!isStreaming && onThinkingLevelChange && (
              <div ref={thinkingDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  style={{
                    display: "flex", alignItems: "center", justifyContent: "center", gap: 5,
                    padding: compactControls ? "0 6px" : "8px 12px",
                    width: compactControls ? "auto" : undefined,
                    height: 32,
                    background: thinkingDropdownOpen ? "var(--bg-hover)" : "none",
                    border: "none",
                    borderRadius: 2,
                    color: thinkingDropdownOpen ? "var(--accent)" : "var(--text-muted)",
                    cursor: isStreaming ? "not-allowed" : "pointer",
                    fontSize: 11,
                    fontWeight: 600,
                    letterSpacing: "0.06em",
                  ...
                  onMouseEnter={(e) => {
                    if (isStreaming) return;
                    e.currentTarget.style.background = "var(--bg-hover)";
                    e.currentTarget.style.color = thinkingDropdownOpen ? "var(--accent)" : "var(--text)";
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = thinkingDropdownOpen ? "var(--bg-hover)" : "none";
                    e.currentTarget.style.color = thinkingDropdownOpen ? "var(--accent)" : "var(--text-muted)";
                  }}
                >
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {(!compactControls || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>{thinkingDisplayLabel}</span>}
                </button>
              ...
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    borderRadius: 2,
                    color: toolDropdownOpen ? "var(--accent)" : "var(--text-muted)",
                    cursor: isStreaming ? "not-allowed" : "pointer",
                    fontSize: 11,
                    fontWeight: 600,
                    letterSpacing: "0.06em",
                  ...
                  onMouseEnter={(e) => {
                    if (isStreaming) return;
                    e.currentTarget.style.background = "var(--bg-hover)";
                    e.currentTarget.style.color = toolDropdownOpen ? "var(--accent)" : "var(--text)";
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = toolDropdownOpen ? "var(--bg-hover)" : "none";
                    e.currentTarget.style.color = toolDropdownOpen ? "var(--accent)" : "var(--text-muted)";
                  }}
                >
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" />
                  </svg>
                  {(!compactControls || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>{toolPresetLabel}</span>}
                </button>
                {toolDropdownOpen && (
                  <div style={{
                    position: "absolute", bottom: "calc(100% + 6px)", right: 0,
            ...
            {!isStreaming && onCompact && (
              <div style={{ position: "relative" }}>
                ...
                >
                  {isCompacting ? (
                    <><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><rect x="2" y="2" width="6" height="6" rx="1" fill="currentColor" /></svg>{(!compactControls || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>Compacting…</span>}</>
                  ) : (
                    <><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <polyline points="4 14 10 14 10 20" /><polyline points="20 10 14 10 14 4" />
                      <line x1="10" y1="14" x2="3" y2="21" /><line x1="21" y1="3" x2="14" y2="10" />
                    </svg>{(!compactControls || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>Compact</span>}</>
                  )}
                </button>
              </div>
            )}

            {isStreaming && isMobile && stopControl}
